...
SPEC = importlib.util.spec_from_file_location(
    'e17_protocol', ROOT / 'tools/e17_protocol.py'
)
...
def test_gate_requires_strong_thresholds_and_e15_improvement():
    rows = {'1': _row(), '2': _row()}
...
def _config():
    grpo = SimpleNamespace(
        monotonic_factorized_landmark_only=True,
        budgeted_factorized_landmark_only=False,
...
@pytest.mark.parametrize('damage', [None, 'frozen', 'partial', 'unchanged',
                                     'no_updates', 'config'])
def test_checkpoint_gate_enforces_e17_contract(monkeypatch, tmp_path, damage):
    spec = importlib.util.spec_from_file_location(
    ...
    checkpoint = dict(state_dict=state, iteration=20,
                      e17_optimizer_updates=20, config=_config())
    if damage == 'frozen':
        ...
        checkpoint['e17_optimizer_updates'] = 19
    elif damage == 'config':
        checkpoint['config'].MODEL.factorized_landmark_monotonic = False
