"""Test E17 orchestration; stubs do not replace Habitat validation."""
...
def _run(tmp_path, fail20=False):
    ...
    shutil.copyfile(ROOT / 'run_r2r/e17_closed_loop_1gpu.bash',
                    tmp_path / 'run_r2r/e17_closed_loop_1gpu.bash')
...
''')
    ...
    env.pop('E17_RUN_ID', None)
    if fail20:
        env['FAIL20'] = '1'
    process = subprocess.run(
        ['bash', 'run_r2r/e17_closed_loop_1gpu.bash',
         '0', '0', '1', '4', '3134'],
...
def test_runner_uses_monotonic_contract_and_independent_training(tmp_path):
    ...
    for call, iterations in zip(training, ['20', '500']):
        ...
        assert argv[
            argv.index('GRPO.monotonic_factorized_landmark_only') + 1
        ] == 'True'
        assert argv[
            argv.index('GRPO.budgeted_factorized_landmark_only') + 1
        ] == 'False'
        assert argv[argv.index('MODEL.factorized_landmark_size') + 1] == '128'
        assert argv[
            argv.index('MODEL.factorized_landmark_monotonic') + 1
        ] == 'True'
    ...
    assert evaluation['gpu'] == '0'
    assert evaluation['argv'][
        evaluation['argv'].index('EVAL.EPISODE_COUNT') + 1
    ] == '-1'
    gate = next(call for call in calls
                if call['argv'][:2] == ['tools/e17_protocol.py', 'results'])
    assert gate['argv'][gate['argv'].index('--output') + 1].endswith(
        'e17_r2r_monotonic_factorized_landmark/full_result.json'
    )
    status = tmp_path / (
        'data/logs/e17_r2r_monotonic_factorized_landmark/exit_status.txt'
    )
