"""Test E16 orchestration; stubs do not replace Habitat validation."""
...
def _run(tmp_path, fail20=False):
    ...
    shutil.copyfile(ROOT / 'run_r2r/e16_closed_loop_1gpu.bash',
                    tmp_path / 'run_r2r/e16_closed_loop_1gpu.bash')
...
''')
    ...
    env.pop('E16_RUN_ID', None)
    if fail20:
        env['FAIL20'] = '1'
    process = subprocess.run(
        ['bash', 'run_r2r/e16_closed_loop_1gpu.bash',
         '1', '2', '1', '4', '2934'],
...
def test_runner_uses_budgeted_contract_and_independent_training(tmp_path):
    ...
    for call, iterations in zip(training, ['20', '500']):
        ...
        assert argv[argv.index('GRPO.factorized_landmark_only') + 1] == 'False'
        assert argv[argv.index('MODEL.factorized_landmark_size') + 1] == '128'
        assert argv[
            argv.index('MODEL.factorized_landmark_budgeted') + 1
        ] == 'True'
    ...
    gate = next(call for call in calls
                if call['argv'][:2] == ['tools/e16_protocol.py', 'results'])
    assert gate['argv'][gate['argv'].index('--output') + 1].endswith(
        'e16_r2r_budgeted_factorized_landmark/full_result.json'
    )
    status = tmp_path / (
        'data/logs/e16_r2r_budgeted_factorized_landmark/exit_status.txt'
    )
