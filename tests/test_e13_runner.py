"""Test E13 orchestration; stubs do not replace Habitat validation."""
...
def _run(tmp_path, fail20=False):
    ...
    shutil.copyfile(
        ROOT / 'run_r2r/e13_closed_loop_1gpu.bash',
        tmp_path / 'run_r2r/e13_closed_loop_1gpu.bash',
    )
...
''')
    ...
    env.pop('E13_RUN_ID', None)
    if fail20:
        env['FAIL20'] = '1'
    process = subprocess.run(
        ['bash', 'run_r2r/e13_closed_loop_1gpu.bash',
         '1', '2', '1', '4', '2534'],
...
def test_runner_uses_fixed_contract_and_independent_training(tmp_path):
    ...
    for call, iterations in zip(training, ['20', '500']):
        ...
        assert argv[argv.index('GRPO.instruction_coverage_only') + 1] == 'True'
        assert argv[argv.index('GRPO.successor_only') + 1] == 'False'
        assert argv[argv.index('GRPO.grpo_beta') + 1] == '0.04'
        assert argv[argv.index('GRPO.is_requeue') + 1] == 'False'
        assert call['gpu'] == '1'
    evaluation = next(
        call for call in calls if 'EVAL.EPISODE_COUNT' in call['argv']
    )
    assert evaluation['gpu'] == '2'
    argv = evaluation['argv']
    assert argv[argv.index('EVAL.EPISODE_COUNT') + 1] == '-1'
    assert argv[argv.index('EVAL.CKPT_PATH_DIR') + 1].endswith(
        '_500/ckpt.iter500.pth'
    )
    status = tmp_path / 'data/logs/e13_r2r_instruction_coverage/exit_status.txt'
