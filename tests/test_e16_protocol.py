...
SPEC = importlib.util.spec_from_file_location(
    'e16_protocol', ROOT / 'tools/e16_protocol.py'
)
...
def test_gate_rejects_a_strong_threshold_pass_that_regresses_e15():
    rows = {'1': _row(oracle_success=0.715)}

    report = PROTOCOL.evaluate_results(rows, {'1'})

    assert all(report['passes'].values())
    assert not report['e15_passes']['OSR']
    assert report['decision'] == 'no-go'


def _config():
    grpo = SimpleNamespace(
        budgeted_factorized_landmark_only=True,
        factorized_landmark_only=False, landmark_transport_only=False,
    ...
    return SimpleNamespace(
        ...
        MODEL=SimpleNamespace(
            ...
            instruction_coverage_hidden_size=0, landmark_transport_size=0,
            factorized_landmark_size=128,
            factorized_landmark_budgeted=True,
        ),
...
@pytest.mark.parametrize('damage', [None, 'frozen', 'partial', 'unchanged',
                                     'no_updates', 'config'])
def test_checkpoint_gate_enforces_e16_contract(monkeypatch, tmp_path, damage):
    spec = importlib.util.spec_from_file_location(
    ...
    state = {'net.vln_bert.factorized_landmark.' + key: value
             for key, value in transport.items()}
    state['net.frozen'] = torch.ones(2)
    checkpoint = dict(state_dict=state, iteration=20,
                      e16_optimizer_updates=20, config=_config())
    if damage == 'frozen':
        state['net.frozen'].zero_()
    elif damage == 'partial':
        del state['net.vln_bert.factorized_landmark.view_projection.weight']
    elif damage == 'unchanged':
        transport['output.bias'].zero_()
    elif damage == 'no_updates':
        checkpoint['e16_optimizer_updates'] = 19
    elif damage == 'config':
        checkpoint['config'].MODEL.factorized_landmark_budgeted = False
