"""Exercise E14 transport injection through the real navigation model."""
...
def test_default_config_registers_e14_keys():
    source = (ROOT / 'vlnce_baselines/config/default.py').read_text()
    assert '_C.GRPO.landmark_transport_only = False' in source
    assert '_C.MODEL.landmark_transport_size = 0' in source


def test_vlnbert_loader_propagates_and_zero_initializes_e14():
    source = (
        ROOT / 'vlnce_baselines/models/etp/ETP_R1_vlnbert_init.py'
    ).read_text()
    assert 'vis_config.landmark_transport_size = getattr(' in source
    assert 'visual_model.landmark_transport.reset_output()' in source
...
def model(monkeypatch):
    ...
    config = SimpleNamespace(
        ...
        successor_hidden_size=0, landmark_transport_size=8,
    )
...
def test_zero_transport_is_exact_e0_before_graph_encoder(model):
    ...
    assert torch.equal(torch.isfinite(enabled['global_logits']),
                       torch.isfinite(disabled['global_logits']))
    assert enabled['transport_residual_norm'].item() == 0.0


def test_navigation_objective_reaches_only_e14(model):
    inputs = navigation_inputs()
    ...
    assert model.landmark_transport.view_projection.weight.grad.abs().sum() > 0
    assert model.landmark_transport.text_projection.weight.grad.abs().sum() > 0
    assert model.landmark_transport.output.weight.grad.abs().sum() > 0
    assert all(
        parameter.grad is None
        for name, parameter in model.named_parameters()
        if not name.startswith('landmark_transport.')
    )


def test_model_rejects_combining_e14_with_e13(model):
    config = SimpleNamespace(**vars(model.config))
    config.instruction_coverage_hidden_size = 32
