...
SPEC = importlib.util.spec_from_file_location(
    'e15_protocol', ROOT / 'tools/e15_protocol.py'
)
...
def _config():
    grpo = SimpleNamespace(
        factorized_landmark_only=True, landmark_transport_only=False,
...
@pytest.mark.parametrize('damage', [None, 'frozen', 'partial', 'unchanged',
                                     'no_updates', 'config'])
def test_checkpoint_gate_enforces_e15_contract(monkeypatch, tmp_path, damage):
    spec = importlib.util.spec_from_file_location(
    ...
    checkpoint = dict(state_dict=state, iteration=20,
                      e15_optimizer_updates=20, config=_config())
    if damage == 'frozen':
        ...
        checkpoint['e15_optimizer_updates'] = 19
    elif damage == 'config':
