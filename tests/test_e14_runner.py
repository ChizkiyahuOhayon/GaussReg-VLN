"""Test E14 orchestration; stubs do not replace Habitat validation."""
...
def _run(tmp_path, fail20=False):
    ...
    shutil.copyfile(ROOT / 'run_r2r/e14_closed_loop_1gpu.bash',
                    tmp_path / 'run_r2r/e14_closed_loop_1gpu.bash')
...
''')
    ...
    env.pop('E14_RUN_ID', None)
    if fail20:
        env['FAIL20'] = '1'
    process = subprocess.run(
        ['bash', 'run_r2r/e14_closed_loop_1gpu.bash',
         '1', '2', '1', '4', '2634'],
...
def test_runner_uses_fixed_contract_and_independent_training(tmp_path):
    ...
    for call, iterations in zip(training, ['20', '500']):
        ...
        assert argv[argv.index('GRPO.landmark_transport_only') + 1] == 'True'
        assert argv[argv.index('MODEL.landmark_transport_size') + 1] == '128'
        assert argv[argv.index('GRPO.instruction_coverage_only') + 1] == 'False'
    ...
    gate = next(call for call in calls
                if call['argv'][:2] == ['tools/e14_protocol.py', 'results'])
    assert gate['argv'][gate['argv'].index('--output') + 1].endswith(
        'e14_r2r_landmark_transport/full_result.json'
    )
    status = tmp_path / 'data/logs/e14_r2r_landmark_transport/exit_status.txt'
