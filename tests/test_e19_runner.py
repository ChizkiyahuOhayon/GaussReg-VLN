"""Test E19 orchestration; stubs do not replace Habitat validation."""
...
def run(tmp_path, fail20=False):
    (tmp_path / 'run_r2r').mkdir()
    shutil.copyfile(ROOT / 'run_r2r/e19_closed_loop_1gpu.bash',
                    tmp_path / 'run_r2r/e19_closed_loop_1gpu.bash')
...
''')
    ...
    env.pop('E19_RUN_ID', None)
    if fail20:
        env['FAIL20'] = '1'
    process = subprocess.run(
        ['bash', 'run_r2r/e19_closed_loop_1gpu.bash',
         '0', '1', '1', '4', '2934'],
...
def test_runner_uses_fixed_contract_and_independent_training(tmp_path):
    ...
    for call, iterations in zip(training, ['20', '500']):
        ...
        assert argv[argv.index('GRPO.factorized_geometry_only') + 1] == 'True'
        assert argv[argv.index('MODEL.factorized_landmark_size') + 1] == '128'
        assert argv[argv.index('MODEL.transient_geometry_size') + 1] == '128'
    ...
    assert evaluation['gpu'] == '1'
    assert evaluation['argv'][
        evaluation['argv'].index('EVAL.EPISODE_COUNT') + 1
    ] == '-1'
    status = tmp_path / (
        'data/logs/e19_r2r_factorized_semantic_geometry/exit_status.txt'
    )
...
def test_smoke_failure_stops_before_main_training_and_evaluation(tmp_path):
    process, calls = run(tmp_path, fail20=True)
