"""Exercise E17 monotonic routing through the real navigation model."""
...
def test_default_config_registers_e17_keys():
    source = (ROOT / 'vlnce_baselines/config/default.py').read_text()
    assert '_C.GRPO.monotonic_factorized_landmark_only = False' in source
    assert '_C.MODEL.factorized_landmark_monotonic = False' in source


def test_vlnbert_loader_propagates_e17_mode():
    source = (
        ROOT / 'vlnce_baselines/models/etp/ETP_R1_vlnbert_init.py'
    ).read_text()
    assert 'vis_config.factorized_landmark_monotonic = getattr(' in source
...
def model(monkeypatch):
    ...
    load('vlnce_baselines.monotonic_factorized_landmark',
         'vlnce_baselines/monotonic_factorized_landmark.py')
    module = load('e17_real_forward',
    ...
    config = SimpleNamespace(
        ...
        factorized_landmark_size=8, factorized_landmark_budgeted=False,
        factorized_landmark_monotonic=True,
    )
...
def navigation_inputs(width=16):
    ...
    return dict(
        ...
        gmap_pair_dists=torch.zeros(1, 5, 5),
...
def test_zero_monotonic_transport_is_e0_and_reports_progress(model):
    ...
    assert enabled['instruction_stage_progress'].shape == (1, 4)
    assert enabled['instruction_stage_progress'].sum().item() == pytest.approx(1)


def test_monotonic_routing_preserves_stop_and_only_trains_e17(model):
    inputs = navigation_inputs()
    ...
    assert not torch.allclose(
        routed_probs[:, 3:], base_probs[:, 3:], atol=1e-7
    )
...
def test_monotonic_and_budgeted_modes_are_exclusive(model):
    config = SimpleNamespace(**vars(model.config))
    config.factorized_landmark_budgeted = True
