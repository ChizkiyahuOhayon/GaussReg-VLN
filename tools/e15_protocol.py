#!/usr/bin/env python3
"""E15 provenance, strict checkpoint contract and full-evaluation gate."""
...
def preflight(args):
    ...
    assets = [
        ...
        'run_r2r/e15_closed_loop_1gpu.bash',
        'docs/e15_factorized_landmark.md',
    ]
    if subprocess.check_output(
            ['git', 'diff', 'HEAD', '--name-only'],
            universal_newlines=True).strip():
        raise RuntimeError('Commit tracked code changes before a reproducible E15 run')
    report = {
        'experiment': 'E15-FLR',
    ...
    print('E15_PREFLIGHT_OK ' + json.dumps(report, sort_keys=True))
...
def check_checkpoint(args):
    ...
    if set(frozen) != set(base) or any(
        ...
        raise RuntimeError('E15 changed frozen E0 tensors or used a wrong baseline')
    ...
    if set(transport) != set(expected) or any(
        ...
        raise RuntimeError('E15 transport is missing, partial or misconfigured')
    if sum(tensor.numel() for tensor in transport.values()) != 590848:
        raise RuntimeError('E15 checkpoint has the wrong transport parameter count')
    if any(not torch.isfinite(tensor).all() for tensor in state.values()):
        raise RuntimeError('E15 checkpoint contains nonfinite tensors')
    if (checkpoint.get('iteration') != args.iteration or
            checkpoint.get('e15_optimizer_updates') != args.iteration):
        raise RuntimeError('E15 iteration or optimizer-update count is wrong')
    if (torch.count_nonzero(transport['output.weight']).item() == 0 and
            torch.count_nonzero(transport['output.bias']).item() == 0):
        raise RuntimeError('E15 output stayed at its zero initialization')
    ...
    if (not config.GRPO.factorized_landmark_only or
        ...
        raise RuntimeError('Checkpoint training configuration differs from E15 protocol')
    print('E15_CHECKPOINT_OK iteration=%d updates=%d parameters=590848 '
          'frozen_E0_equal=1 sha256=%s' % (
              args.iteration, checkpoint['e15_optimizer_updates'],
...
def results(args):
    ...
    report.update({
        ...
        'dataset_sha256': sha256(path), 'experiment': 'E15-FLR',
        'utc': datetime.now(timezone.utc).isoformat(),
    })
    Path(args.output).write_text(json.dumps(report, indent=2) + '\n')
    print('E15_FULL_EVAL ' + json.dumps(report, sort_keys=True))
