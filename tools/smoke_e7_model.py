...
def main():
    parser = argparse.ArgumentParser(
        description='Verify the production ETP-R1 terminal-commit model.'
    )
    ...
    if not torch.equal(outputs['global_logits'], baseline_logits):
        raise RuntimeError('E7 changed the E0 navigation logits')
    ...
    if not torch.equal(
        ...
        raise RuntimeError('E7 does not reproduce E0 terminal ranking')
    if not torch.isneginf(commit_logits[gmap_visited_masks.logical_not()]).all():
        raise RuntimeError('E7 exposed a non-visited terminal action')
    ...
    if backbone_gradients:
        raise RuntimeError('Terminal-commit loss reached the frozen backbone')
    if torch.count_nonzero(head.output.weight.grad).item() == 0:
        raise RuntimeError('Terminal-commit output has no gradient')

    print(
        'E7_MODEL_SMOKE_PASSED trainable=%d terminal_mismatch=0 '
        'navigation_mismatch=0 backbone_gradient=0' % parameter_count
    )
