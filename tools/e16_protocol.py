#!/usr/bin/env python3
"""E16 provenance, strict checkpoint contract and full-evaluation gate."""
...
from tools import e15_protocol as shared

E15_REFERENCE = {
    'NE': 3.931522907625404,
    'OSR': 71.72376291462751,
    'SR': 65.47036432843937,
    'SPL': 55.84626062781841,
    'nDTW': 66.02703298613687,
    'SDTW': 54.024968699571744,
}


def evaluate_results(rows, expected_ids):
    report = shared.evaluate_results(rows, expected_ids)
    e15_passes = {
        key: report['metrics'][key] < value if key == 'NE'
        else report['metrics'][key] > value
        for key, value in E15_REFERENCE.items()
    }
    report['e15_reference'] = E15_REFERENCE
    report['e15_passes'] = e15_passes
    report['decision'] = (
        'go' if all(report['passes'].values()) and all(e15_passes.values())
        else 'no-go'
    )
    return report
...
def preflight(args):
    ...
    train_path, train = shared.dataset_episodes('train', '_10')
    eval_path, evaluation = shared.dataset_episodes('val_unseen')
    ...
    assets = [
        ...
        shared.DATASET / 'val_unseen/val_unseen_gt.json.gz',
        ...
        'run_r2r/e16_closed_loop_1gpu.bash',
        'tools/e16_protocol.py', 'tools/e15_protocol.py',
        'tools/smoke_e16_model.py', 'tools/smoke_e15_model.py',
        'vlnce_baselines/budgeted_factorized_landmark.py',
        'vlnce_baselines/factorized_landmark.py',
        'docs/e16_budgeted_factorized_landmark.md',
    ]
    if subprocess.check_output(
            ['git', 'diff', 'HEAD', '--name-only'],
            universal_newlines=True).strip():
        raise RuntimeError('Commit tracked code changes before a reproducible E16 run')
    report = {
        'experiment': 'E16-BFLR',
        ...
        'asset_sha256': {str(path): shared.sha256(path) for path in assets},
    }
    Path(args.output).write_text(json.dumps(report, indent=2) + '\n')
    print('E16_PREFLIGHT_OK ' + json.dumps(report, sort_keys=True))
...
def check_checkpoint(args):
    ...
    if set(frozen) != set(base) or any(
        ...
        raise RuntimeError('E16 changed frozen E0 tensors or used a wrong baseline')
    ...
    if set(transport) != set(expected) or any(
        ...
        raise RuntimeError('E16 transport is missing, partial or misconfigured')
    if sum(tensor.numel() for tensor in transport.values()) != 590848:
        raise RuntimeError('E16 checkpoint has the wrong transport parameter count')
    if any(not torch.isfinite(tensor).all() for tensor in state.values()):
        raise RuntimeError('E16 checkpoint contains nonfinite tensors')
    if (checkpoint.get('iteration') != args.iteration or
            checkpoint.get('e16_optimizer_updates') != args.iteration):
        raise RuntimeError('E16 iteration or optimizer-update count is wrong')
    if (torch.count_nonzero(transport['output.weight']).item() == 0 and
            torch.count_nonzero(transport['output.bias']).item() == 0):
        raise RuntimeError('E16 output stayed at its zero initialization')
    ...
    if (not config.GRPO.budgeted_factorized_landmark_only or
            ...
            not config.MODEL.factorized_landmark_budgeted or
        ...
        raise RuntimeError('Checkpoint training configuration differs from E16 protocol')
    print('E16_CHECKPOINT_OK iteration=%d updates=%d parameters=590848 '
          'frozen_E0_equal=1 sha256=%s' % (
              args.iteration, checkpoint['e16_optimizer_updates'],
              shared.sha256(args.checkpoint)
          ))


def results(args):
    dataset_path, episodes = shared.dataset_episodes('val_unseen')
    ...
    for display, key in shared.METRICS.items():
    ...
    report.update({
        ...
        'checkpoint_sha256': shared.sha256(args.checkpoint),
        'per_episode_file': str(files[0]),
        'per_episode_sha256': shared.sha256(files[0]),
        'dataset_sha256': shared.sha256(dataset_path),
        'experiment': 'E16-BFLR',
        'reference': 'E15-FLR',
        'utc': datetime.now(timezone.utc).isoformat(),
    })
    Path(args.output).write_text(json.dumps(report, indent=2) + '\n')
    print('E16_FULL_EVAL ' + json.dumps(report, sort_keys=True))
