#!/usr/bin/env python3
"""E13 provenance, strict checkpoint contract and full-evaluation gate."""
...
def preflight(args):
    ...
    assets = [
        ...
        'run_r2r/e13_closed_loop_1gpu.bash',
    ]
    if subprocess.check_output(
            ['git', 'diff', 'HEAD', '--name-only'],
            universal_newlines=True).strip():
        raise RuntimeError('Commit tracked code changes before a reproducible E13 run')
    report = {
        'experiment': 'E13-IIEG',
    ...
    print('E13_PREFLIGHT_OK ' + json.dumps(report, sort_keys=True))
...
def check_checkpoint(args):
    ...
    state = {
        key.replace('net.module.', 'net.'): value
    ...
    prefix = 'net.vln_bert.instruction_coverage.'
    head = {
    ...
    if set(frozen) != set(base) or any(
        ...
        raise RuntimeError('E13 changed frozen E0 tensors or used a wrong baseline')
    expected = module.InstructionCoverageResidual(32).state_dict()
    if set(head) != set(expected) or any(
            head[key].shape != expected[key].shape for key in expected):
        raise RuntimeError('E13 head is missing, partial or misconfigured')
    if sum(tensor.numel() for tensor in head.values()) != 449:
        raise RuntimeError('E13 checkpoint does not contain exactly 449 head parameters')
    if any(not torch.isfinite(tensor).all() for tensor in state.values()):
        raise RuntimeError('E13 checkpoint contains nonfinite tensors')
    if (checkpoint.get('iteration') != args.iteration or
            checkpoint.get('e13_optimizer_updates') != args.iteration):
        raise RuntimeError('E13 iteration or optimizer-update count is wrong')
    if (torch.count_nonzero(head['output.weight']).item() == 0 and
            torch.count_nonzero(head['output.bias']).item() == 0):
        raise RuntimeError('E13 output stayed at its zero initialization')
    ...
    old_modes = [
        ...
        'geo_token_only', 'successor_only',
    ]
    if (not config.GRPO.instruction_coverage_only or
            any(getattr(config.GRPO, name, False) for name in old_modes) or
            config.MODEL.instruction_coverage_hidden_size != 32 or
            config.MODEL.successor_hidden_size != 0 or
        ...
        raise RuntimeError('Checkpoint training configuration differs from E13 protocol')
    print('E13_CHECKPOINT_OK iteration=%d updates=%d head_parameters=449 '
          'frozen_E0_equal=1 sha256=%s' % (
              args.iteration, checkpoint['e13_optimizer_updates'],
...
def results(args):
    ...
    report.update({
        ...
        'dataset_sha256': sha256(path), 'experiment': 'E13-IIEG',
        'utc': datetime.now(timezone.utc).isoformat(),
    })
    Path(args.output).write_text(json.dumps(report, indent=2) + '\n')
    print('E13_FULL_EVAL ' + json.dumps(report, sort_keys=True))
