...
# Train and evaluate sequentially; each child sees its selected GPU as device 0.
...
EVAL_ENVS=${4:-4}
PORT=${5:-2934}
RUN_ID=${E19_RUN_ID:-e19_r2r_factorized_semantic_geometry}
if [[ ! "${RUN_ID}" =~ ^[a-zA-Z0-9_-]+$ ]]; then
    echo 'E19_RUN_ID must contain only letters, digits, underscores or hyphens.' >&2
    exit 2
fi
...
for directory in "${LOG_DIR}" "data/logs/checkpoints/${SMOKE_EXP}" \
                 "data/logs/checkpoints/${MAIN_EXP}" \
                 "data/logs/checkpoints/${EVAL_EXP}"; do
    if [ -e "${directory}" ]; then
        echo "Refusing to overwrite existing evidence: ${directory}" >&2
        echo 'Use a new E19_RUN_ID for a retry; training still starts from E0.' >&2
        exit 1
    fi
done
mkdir -p "${LOG_DIR}"
...
COMMON=(
    ...
    MODEL.landmark_transport_size 0 MODEL.factorized_landmark_size 128
    MODEL.factorized_landmark_budgeted False
    MODEL.factorized_landmark_monotonic False
    MODEL.transient_geometry_size 128 MODEL.route_attention False
    MODEL.pretrained_path "${PRETRAINED}"
)

echo '[1/7] Record assets, dataset identities and code revision'
CUDA_VISIBLE_DEVICES=${TRAIN_GPU} python tools/e19_protocol.py preflight \
    --baseline "${BASE}" --pretrained "${PRETRAINED}" \
    --output "${LOG_DIR}/manifest.json" 2>&1 | tee "${LOG_DIR}/preflight.log"

phase=model_smoke
echo '[2/7] Production model, strict E0 load and semantic-geometry smoke'
CUDA_VISIBLE_DEVICES=${TRAIN_GPU} python tools/smoke_e19_model.py \
    --baseline "${BASE}" --pretrained "${PRETRAINED}" \
...
train_from_e0() {
    ...
    CUDA_VISIBLE_DEVICES=${TRAIN_GPU} python -m torch.distributed.launch \
        ...
        GRPO.is_requeue False GRPO.factorized_geometry_only True \
        ...
        GRPO.landmark_transport_only False GRPO.route_attention_only False \
        ...
        GRPO.successor_only False GRPO.instruction_coverage_only False \
        GRPO.back_algo control GRPO.batch_size 1 GRPO.sample_num 8 \
        GRPO.update_epochs 1 GRPO.grpo_beta 0.04 GRPO.enable_amp False \
...
phase=train20
echo '[3/7] Independent 20-iteration training smoke from strict E0'
train_from_e0 20 "${SMOKE_EXP}" "${PORT}" 2>&1 | tee "${LOG_DIR}/train20.log"
phase=check20
echo '[4/7] Require both learned branches and bitwise unchanged E0'
python tools/e19_protocol.py checkpoint "${SMOKE_CKPT}" --baseline "${BASE}" \
...
python tools/e19_protocol.py checkpoint "${MAIN_CKPT}" --baseline "${BASE}" \
...
phase=gate
python tools/e19_protocol.py results "${RESULTS}" \
    --checkpoint "${MAIN_CKPT}" --output "${LOG_DIR}/full_result.json" \
    2>&1 | tee "${LOG_DIR}/gate.log"
phase=completed
echo "E19 execution completed. Performance decision: ${LOG_DIR}/full_result.json"
