...
EVAL_ENVS=${4:-4}
PORT=${5:-2534}
RUN_ID=${E13_RUN_ID:-e13_r2r_instruction_coverage}
if [[ ! "${RUN_ID}" =~ ^[a-zA-Z0-9_-]+$ ]]; then
    echo 'E13_RUN_ID must contain only letters, digits, underscores or hyphens.' >&2
    exit 2
fi
...
for directory in "${LOG_DIR}" "data/logs/checkpoints/${SMOKE_EXP}" \
    ...
    if [ -e "${directory}" ]; then
        ...
        echo 'For an infrastructure retry, use a new E13_RUN_ID; training still starts from E0.' >&2
        exit 1
    fi
done
mkdir -p "${LOG_DIR}"
...
COMMON=(
    ...
    MODEL.successor_hidden_size 0 MODEL.instruction_coverage_hidden_size 32
    MODEL.pretrained_path "${PRETRAINED}"
)

echo '[1/7] Record assets, dataset identities and code revision'
CUDA_VISIBLE_DEVICES=${TRAIN_GPU} python tools/e13_protocol.py preflight \
...
CUDA_VISIBLE_DEVICES=${TRAIN_GPU} python tools/smoke_e13_model.py \
    --baseline "${BASE}" --pretrained "${PRETRAINED}" \
...
train_from_e0() {
    ...
    CUDA_VISIBLE_DEVICES=${TRAIN_GPU} python -m torch.distributed.launch \
        ...
        GRPO.is_requeue False GRPO.instruction_coverage_only True \
        ...
        GRPO.successor_only False GRPO.back_algo control \
...
phase=check20
echo '[4/7] Require a changed 449-parameter head and bitwise unchanged E0'
python tools/e13_protocol.py checkpoint "${SMOKE_CKPT}" --baseline "${BASE}" \
...
python tools/e13_protocol.py checkpoint "${MAIN_CKPT}" --baseline "${BASE}" \
...
python tools/e13_protocol.py results "${RESULTS}" \
    --checkpoint "${MAIN_CKPT}" --output "${LOG_DIR}/full_result.json" \
    2>&1 | tee "${LOG_DIR}/gate.log"
phase=completed
echo "E13 execution completed. Performance decision: ${LOG_DIR}/full_result.json"
