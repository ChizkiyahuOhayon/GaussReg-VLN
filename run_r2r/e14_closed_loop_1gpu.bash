...
EVAL_ENVS=${4:-4}
PORT=${5:-2634}
RUN_ID=${E14_RUN_ID:-e14_r2r_landmark_transport}
if [[ ! "${RUN_ID}" =~ ^[a-zA-Z0-9_-]+$ ]]; then
    echo 'E14_RUN_ID must contain only letters, digits, underscores or hyphens.' >&2
    exit 2
fi
...
for directory in "${LOG_DIR}" "data/logs/checkpoints/${SMOKE_EXP}" \
    ...
    if [ -e "${directory}" ]; then
        ...
        echo 'For an infrastructure retry, use a new E14_RUN_ID; training still starts from E0.' >&2
        exit 1
    fi
done
mkdir -p "${LOG_DIR}"
...
CUDA_VISIBLE_DEVICES=${TRAIN_GPU} python tools/e14_protocol.py preflight \
...
CUDA_VISIBLE_DEVICES=${TRAIN_GPU} python tools/smoke_e14_model.py \
    --baseline "${BASE}" --pretrained "${PRETRAINED}" \
...
train_from_e0() {
    ...
    CUDA_VISIBLE_DEVICES=${TRAIN_GPU} python -m torch.distributed.launch \
        ...
        GRPO.is_requeue False GRPO.landmark_transport_only True \
...
phase=check20
echo '[4/7] Require changed transport weights and bitwise unchanged E0'
python tools/e14_protocol.py checkpoint "${SMOKE_CKPT}" --baseline "${BASE}" \
...
python tools/e14_protocol.py checkpoint "${MAIN_CKPT}" --baseline "${BASE}" \
...
phase=gate
python tools/e14_protocol.py results "${RESULTS}" \
    --checkpoint "${MAIN_CKPT}" --output "${LOG_DIR}/full_result.json" \
    2>&1 | tee "${LOG_DIR}/gate.log"
phase=completed
echo "E14 execution completed. Performance decision: ${LOG_DIR}/full_result.json"
